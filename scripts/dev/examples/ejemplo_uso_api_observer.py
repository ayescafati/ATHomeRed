...
def main():
    ...
    if not auth_response:
        ...
        print(
            '   {"email": "paciente.test@athomered.com", "password": "password123", "nombre": "Juan Test"}'
        )
        return
    ...
    filtros = {"especialidad_id": 1, "provincia": "Buenos Aires"}  # ID de Enfermería

    profesionales = hacer_request(
        "GET", "/busqueda/profesionales", token=paciente_token, params=filtros
    )
    ...
    print(
        f"      Especialidad: {profesional.get('especialidad', {}).get('nombre', 'N/A')}"
    )
    ...
    cita_creada = hacer_request(
        "POST", "/consultas/", token=paciente_token, json_data=nueva_cita
    )
    ...
    print(
        f"      Fecha: {cita_creada['fecha']} {cita_creada['hora_inicio']}-{cita_creada['hora_fin']}"
    )
    ...
    confirmacion = hacer_request(
        "POST", f"/consultas/{cita_id}/confirmar", token=paciente_token
    )
    ...
    mis_citas = hacer_request(
        "GET",
        f"/consultas/profesional/{profesional_id}",
        token=prof_token,
    ...
    if mis_citas:
        ...
        for i, c in enumerate(mis_citas[:3], 1):
            print(
                f"      {i}. ID: {c['id'][:8]}... | Estado: {c['estado']} | Fecha: {c['fecha']}"
            )
    ...
    cita_completada = hacer_request(
        "POST",
        f"/consultas/{cita_id}/completar",
        token=prof_token,
        json_data=completar_data,
    )
    ...
    print(
        "   • El patrón Observer desacopla la lógica de negocio de las notificaciones"
    )
    print("\n" + "=" * 80)
