...
def agregar_matriculas():
    ...
    try:
        ...
        prov = session.execute(text("SELECT id FROM athome.provincia WHERE nombre = :n LIMIT 1"), {"n": "Córdoba"}).fetchone()
        if not prov:
            prov_id = str(uuid4())
            session.execute(text("INSERT INTO athome.provincia (id, nombre) VALUES (:id, :nombre)"), {"id": prov_id, "nombre": "Córdoba"})
            session.commit()
            prov = session.execute(text("SELECT id FROM athome.provincia WHERE nombre = :n LIMIT 1"), {"n": "Córdoba"}).fetchone()
        ...
        for idx, p in enumerate(profs, 1):
            pid = str(p[0])
            exists = session.execute(text("SELECT id FROM athome.matricula WHERE profesional_id = :pid LIMIT 1"), {"pid": pid}).fetchone()
            if exists:
                continue
            nro = f"AUTO-{idx:04d}"
            session.execute(
                ...
                {"id": str(uuid4()), "pid": pid, "prov": provincia_id, "nro": nro, "desde": hoy, "hasta": hoy + timedelta(days=3650)}
            )
            created += 1
        session.commit()
