...
from app.infra.persistence.database import ENGINE, SessionLocal
...
PROVINCIAS = [
    {"nombre": "Córdoba", "departamentos": ["Capital", "Colón", "Punilla", "Santa María"]},
    {"nombre": "Buenos Aires", "departamentos": ["La Plata", "San Isidro", "Vicente López", "Lomas de Zamora"]},
    {"nombre": "Santa Fe", "departamentos": ["La Capital", "Rosario", "Castellanos"]},
]

BARRIOS_CORDOBA = [
    "Nueva Córdoba", "Centro", "Alberdi", "General Paz", "Güemes",
    "Villa Carlos Paz", "Alta Córdoba", "Cerro de las Rosas", "Arguello"
]
...
PROFESIONALES = [
    ...
    },
    {
        ...
        "celular": "351-5551005",
        "especialidades": ["Apoyo a Personas con Discapacidad", "Rehabilitación Domiciliaria"],
        "matricula": "AT-5567",
        "barrio": "Güemes",
    },
    {
    ...
    },
    {
        ...
        "celular": "351-5551007",
        "especialidades": ["Acompañamiento Terapéutico", "Acompañamiento en Salud Mental"],
        "matricula": "AT-3456",
...
def crear_especialidades(session: Session) -> dict:
    ...
    for esp_data in ESPECIALIDADES:
        ...
        result = session.execute(
            text("SELECT id_especialidad FROM athome.especialidad WHERE nombre = :nombre"),
...
def crear_profesionales(session: Session, especialidades_map: dict, ubicaciones: dict) -> list:
    ...
    for idx, prof_data in enumerate(PROFESIONALES, 1):
        ...
        if existing:
            ...
            if prof_result:
                ...
                print(f"  {idx}. ℹ {prof_data['nombre']} {prof_data['apellido']} ya existe")
                continue
        ...
        if not provincia_id:
            ...
            print(f"    ⚠ No se pudo crear matrícula (falta provincia)")
        else:
            nro_matricula = prof_data.get("matricula", f"PROF-{idx:04d}")
            exists = session.execute(
                text("SELECT id FROM athome.matricula WHERE profesional_id = :pid AND nro_matricula = :nro"),
        ...
        print(f"  {idx}. ✓ {prof_data['nombre']} {prof_data['apellido']} "
...
def crear_disponibilidades(session: Session, profesionales_ids: list):
    ...
    horarios = [
        ...
        {"dias": "lunes,martes,miércoles,jueves,viernes", "inicio": time(8, 0), "fin": time(12, 0)},
    ...
    for idx, data in enumerate(SOLICITANTES_PACIENTES, 1):
        ...
        if existing:
            ...
            if sol_result:
                ...
                print(f"  {idx}. ℹ Solicitante: {sol_data['nombre']} {sol_data['apellido']} ya existe")
                continue
        ...
        for pac_data in data["pacientes"]:
            ...
            relacion_result = session.execute(
                text("SELECT id FROM athome.relacion_solicitante WHERE nombre = :nombre"),
            ...
            print(f"    → Paciente: {pac_data['nombre']} {pac_data['apellido']} "
                  f"({edad} años) - {pac_data['relacion']}")
    
    session.commit()
    print(f"✓ Total: {len(solicitantes_ids)} solicitantes, {len(pacientes_ids)} pacientes\n")
...
def crear_consultas(session: Session, profesionales_ids: list, pacientes_ids: list):
    ...
    for _ in range(20):
        ...
        hora_fin = time(
            (hora_inicio.hour + random.randint(1, 3)) % 24,
            hora_inicio.minute
        )
...
def main(limpiar=False):
    ...
    try:
        ...
        profesionales_ids = crear_profesionales(session, especialidades_map, ubicaciones)
        crear_disponibilidades(session, profesionales_ids)
        solicitantes_ids, pacientes_ids = crear_solicitantes_y_pacientes(session, ubicaciones)
