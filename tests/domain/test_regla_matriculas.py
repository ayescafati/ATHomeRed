...
def test_profesional_con_matricula_es_valido(session, provincia_id):
    ...
    # Limpiar
    session.execute(text("DELETE FROM athome.matricula WHERE profesional_id = :pid"), {"pid": prof_id})
    session.execute(text("DELETE FROM athome.profesional WHERE id = :pid"), {"pid": prof_id})
    session.execute(text("DELETE FROM athome.usuario WHERE id = :uid"), {"uid": user_id})
    session.commit()
