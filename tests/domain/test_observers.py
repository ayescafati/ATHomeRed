...
class TestSubjectObserverPattern:
    ...
    def test_notify_notifica_a_todos_los_observers(self):
        ...
        evento = Event(
            tipo="test.evento", cita_id=uuid4(), datos={"test": "data"}
        )
...
class TestNotificadorEmail:
    ...
    def test_procesa_cita_confirmada(self, capsys):
        ...
        evento = CitaConfirmada(
            cita_id=self.cita_id, confirmado_por="Profesional"
        )
    ...
    def test_procesa_cita_completada(self, capsys):
        ...
        evento = CitaCompletada(
            cita_id=self.cita_id, notas="Consulta exitosa"
        )
...
class TestEventBus:
    ...
    def test_error_en_handler_no_detiene_otros_handlers(self, caplog):
        ...
        assert any("Error procesando evento" in record.message for record in caplog.records)
...
class TestIntegracionObserverEventBus:
    ...
    def test_flujo_completo_notificaciones(self, capsys):
        ...
        evento_cancelada = CitaCancelada(
            cita_id=cita_id, motivo="Test cancelación"
        )
