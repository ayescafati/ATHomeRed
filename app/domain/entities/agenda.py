...
from ..eventos import Event, CitaCreada, CitaConfirmada, CitaCancelada, CitaReprogramada, CitaCompletada
...
class Cita(Subject):
    ...
    def __post_init__(self):
        ...
        duracion = datetime.combine(date.today(), self.hora_fin) - datetime.combine(date.today(), self.hora_inicio)
    ...
    def duracion(self) -> timedelta:
        """Duración de la cita"""
        return datetime.combine(date.today(), self.hora_fin) - datetime.combine(date.today(), self.hora_inicio)
    ...
    def confirmar(self, confirmado_por: str = None) -> None:
        ...
        if self.estado != EstadoCita.PENDIENTE:
            raise ValueError(f"Solo se pueden confirmar citas pendientes. Estado actual: {self.estado.value}")
        
        self.estado = EstadoCita.CONFIRMADA
        self.actualizado_en = datetime.now()
        
        # Notificar a los observadores
        self.notify(CitaConfirmada(
            cita_id=self.id,
    ...
    def completar(self, notas_finales: str = None) -> None:
        ...
        if self.estado != EstadoCita.CONFIRMADA:
            raise ValueError(f"Solo se pueden completar citas confirmadas. Estado actual: {self.estado.value}")
        ...
        # Notificar a los observadores
        self.notify(CitaCompletada(
            cita_id=self.id,
            notas=notas_finales
        ))
    
    def reprogramar(self, nueva_fecha: date, nueva_hora_inicio: time, nueva_hora_fin: time) -> None:
        """
        Reprograma una cita.
        ...
        if not self.puede_modificarse:
            raise ValueError(f"No se puede reprogramar una cita en estado {self.estado.value}")
