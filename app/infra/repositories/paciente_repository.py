...
class PacienteRepository:
    ...
    def obtener_por_id(self, id: UUID) -> Optional[Paciente]:
        ...
        """
        orm = self.session.query(PacienteORM).filter(
            PacienteORM.id == id
        ).first()
    ...
    def listar_por_solicitante(self, solicitante_id: UUID) -> List[Paciente]:
        ...
        Returns:
            Lista de pacientes
        """
        orms = self.session.query(PacienteORM).filter(
            PacienteORM.solicitante_id == solicitante_id
        ).all()
    ...
    def buscar_por_nombre(self, nombre: str, apellido: str = None) -> List[Paciente]:
        ...
        if apellido:
            query = query.filter(
                PacienteORM.apellido.ilike(f"%{apellido}%")
            )
        
        orms = query.all()
        return [self._to_domain(orm) for orm in orms]
    
    def crear(self, paciente: Paciente, solicitante_id: UUID, direccion_id: Optional[UUID] = None) -> Paciente:
        """
        ...
        if direccion_id is None and paciente.ubicacion:
            ...
            direccion_orm = self.direccion_repo.crear_con_jerarquia(paciente.ubicacion)
    ...
    def actualizar(self, paciente: Paciente, direccion_id: Optional[UUID] = None) -> Optional[Paciente]:
        """
