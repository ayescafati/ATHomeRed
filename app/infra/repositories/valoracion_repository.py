...
class ValoracionRepository:
    ...
    def _to_orm(self, valoracion: Valoracion, orm: ValoracionORM = None) -> ValoracionORM:
        """
    ...
    def obtener_por_id(self, id: UUID) -> Optional[Valoracion]:
        ...
        """
        orm = self.session.query(ValoracionORM).filter(
            ValoracionORM.id == id
        ).first()
    ...
    def listar_por_profesional(
        ...
        """
        orms = self.session.query(ValoracionORM).filter(
            ValoracionORM.profesional_id == profesional_id
        ).order_by(
            ValoracionORM.creado_en.desc()
        ).limit(limite).all()
    ...
    def obtener_promedio_profesional(self, profesional_id: UUID) -> float:
        ...
        resultado = self.session.query(
            func.avg(ValoracionORM.puntuacion)
        ).filter(
            ValoracionORM.profesional_id == profesional_id
        ).scalar()
    ...
    def contar_por_profesional(self, profesional_id: UUID) -> int:
        ...
        """
        return self.session.query(ValoracionORM).filter(
            ValoracionORM.profesional_id == profesional_id
        ).count()
    ...
    def actualizar(self, valoracion: Valoracion) -> Optional[Valoracion]:
        ...
        """
        orm = self.session.query(ValoracionORM).filter(
            ValoracionORM.id == valoracion.id
        ).first()
