...
from app.infra.persistence.agenda import ConsultaORM, EventoORM, EstadoConsultaORM
...
class ConsultaRepository:
    ...
    def _to_orm(self, cita: Cita, orm: ConsultaORM = None) -> ConsultaORM:
        ...
        estado_orm = self.session.query(EstadoConsultaORM).filter(
            EstadoConsultaORM.codigo == cita.estado.value
        ).first()
        
        if not estado_orm:
            # Si no existe, crearlo (esto solo debería pasar en desarrollo)
            estado_orm = EstadoConsultaORM(
                codigo=cita.estado.value,
                descripcion=cita.estado.value.upper()
            )
    ...
    def verificar_disponibilidad(
        ...
        citas_existentes = self.session.query(ConsultaORM).join(
            EstadoConsultaORM
        ).filter(
            ConsultaORM.profesional_id == profesional_id,
    ...
    def obtener_eventos(self, cita_id: UUID) -> List[Event]:
        ...
        Returns:
            Lista de eventos
        """
        eventos_orm = self.session.query(EventoORM).filter(
            EventoORM.consulta_id == cita_id
        ).order_by(EventoORM.id).all()
        
        return [
            Event(
                tipo=e.tipo,
                cita_id=e.consulta_id,
                datos=e.datos
            )
