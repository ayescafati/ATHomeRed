...
from sqlalchemy import UniqueConstraint, CheckConstraint, Index, ForeignKey, text, Date
...
class MatriculaORM(Base):
    ...
    profesional_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey(f"{SCHEMA}.profesional.id", ondelete="CASCADE"), nullable=False)
    provincia_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey(f"{SCHEMA}.provincia.id", ondelete="RESTRICT"), nullable=False)
