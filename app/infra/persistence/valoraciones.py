...
from sqlalchemy import  Index, CheckConstraint, ForeignKey, text, Date, Integer, Text
...
class ValoracionORM(Base):
    ...
    __table_args__ = (
        CheckConstraint("puntuacion BETWEEN 1 AND 5", name="ck_valoracion_puntuacion_1_5"),
    ...
    paciente_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey(f"{SCHEMA}.paciente.id", ondelete="CASCADE"), nullable=False
    )
    ...
    paciente: Mapped["PacienteORM"] = relationship("PacienteORM", back_populates="valoraciones")